...
--
-- Name: address_token_balances_two; Type: VIEW; Schema: accounts; Owner: -
--

CREATE VIEW accounts.address_token_balances_two AS
 SELECT x.address_hash,
    x.token_contract_address_hash,
    x.block_number,
    sum(x.amount) AS sum
   FROM ( SELECT xfer.src AS address_hash,
            xfer.contract AS token_contract_address_hash,
            xfer.block_number,
            (- sum(xfer.amount) OVER (PARTITION BY xfer.src, xfer.contract, xfer.block_number ORDER BY xfer.block_number ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW)) AS amount
           FROM accounts.token_value_transfers xfer
        UNION ALL
         SELECT xfer.dst AS address_hash,
            xfer.contract AS token_contract_address_hash,
            xfer.block_number,
            sum(xfer.amount) OVER (PARTITION BY xfer.dst, xfer.contract, xfer.block_number ORDER BY xfer.block_number ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS amount
           FROM accounts.token_value_transfers xfer) x
  GROUP BY x.address_hash, x.token_contract_address_hash, x.block_number;
...
CREATE TABLE public.checked_headers (
    ...
    account_0x46e7be9c627d44914fc15e525c4c2fe073c78f2f integer DEFAULT 0 NOT NULL,
    account_0x2da18f06b0d01b32958a749ee8f91a1261f9eec5 integer DEFAULT 0 NOT NULL,
    account_0x0d17d7d856a3c80f7853d26c9abf979425797391 integer DEFAULT 0 NOT NULL
);
