...
func setInfuraConfig() {
	...
	Infura.AddConfigPath("$GOPATH/src/github.com/vulcanize/account_transformers/environments/")
...
func CleanFullTestDB(db *postgres.DB) {
	...
	db.MustExec("DELETE FROM accounts.addresses")
	db.MustExec("DELETE FROM accounts.eth_balances")
	db.MustExec("DELETE FROM accounts.checked_blocks")
	db.MustExec("DELETE FROM accounts.value_transfer_events")
	db.MustExec("DELETE FROM accounts.topic0_filters")
